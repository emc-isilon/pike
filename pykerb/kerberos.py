...
# GSS mechanism IDs
AUTH_GSS_M_DEFAULT    = 0
AUTH_GSS_M_KERBEROS   = 1
AUTH_GSS_M_NTLM       = 2
     
def authGSSClientInit(service, gssflags=GSS_C_MUTUAL_FLAG|GSS_C_REPLAY_FLAG|GSS_C_CONF_FLAG|GSS_C_INTEG_FLAG):
    """
    ...
    @param user: optional string containing the user name (NTLM only).
    @param password: optional string containing the user password (NTLM only).
    @param domain: optional string containing the user domain (NTLM only).
    @param gssmech: optional integer used to identify the GSS mechanism for credential acquisition.
        (e.g.  AUTH_GSS_M_KERBEROS or AUTH_GSS_M_NTLM)
...
def authGSSClientSessionKey(context):
    """
    Perform the inquiry to obtain the client session key for the now complete GSSAPI client-side operations.
    This method must only be called after authGSSClientStep returns a complete response code.
    Retrieve the key using authGSSClientResponse.
